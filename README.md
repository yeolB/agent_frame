...
운영 프레임은 **Level 1 — Continuity 하나뿐**입니다. 큰 구현 전에는 가벼운 Planner Skill이 현재 해법 자체를 점검하고, 상태가 변경된 Codex 턴은 모델 호출 없이 로컬 스크립트가 셉니다. 짧은 주기에는 메모리를 정리하고, 더 긴 주기에는 깨끗한 컨텍스트의 독립 reviewer가 목표·전략·누적 아키텍처를 함께 검토합니다. 이전 Level 2 architecture 설계는 비활성 archive로 봉인했습니다.
